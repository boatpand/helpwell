...
class EventDetail extends Component {
  ...
  async componentDidMount() {
    ...
    await axios
      .get(`http://localhost:4000/request/request-detail/${id}`)
      .then((res) => {
        this.setState({
          ...
          datet:
            "วันที่ " +
            res.data.date.slice(8, 10) +
            " / " +
            res.data.date.slice(5, 7) +
            " / " +
            res.data.date.slice(0, 4),
    ...
    await axios
      .get(`http://localhost:4000/request/request-detail-detail/${id}`)
      .then((res) => {
        this.setState({
          Detail: res.data,
        });
    ...
    this.setState({
      ...
      victimLocation: {
        lat: parseFloat(this.state.user.Lat),
        lng: parseFloat(this.state.user.Lng),
      },
    });
    if (this.state.Soi == "") {
      this.setState({ show_soi: false });
    }
    if (this.state.Road == "") {
      this.setState({ show_road: false });
    }
  ...
  componentDidUpdate(prevProps, prevState) {
    if (this.state.currentLocation !== prevState.currentLocation) {
      if (this.state.currentLocation.lat !== 0) {
        ...
        const destination = {
          lat: this.state.victimLocation.lat,
          lng: this.state.victimLocation.lng,
        };
  ...
  onSubmit = async (e) => {
    ...
    if (RequestID_Checklist.includes(RequestID_Check)) {
      ...
      await axios
        .post("http://localhost:4000/accept/accept", acceptObject)
      ...
      axios
        .put(
          `http://localhost:4000/request/update-help/${RequestID}`,
          eventObject
        )
        .then((res) => {
      ...
      axios
        .put(
          `http://localhost:4000/request/update-help-detail/${RequestID}`,
          detailObject
        )
        .then((res) => {
      ...
      this.props.history.push({
        pathname: `/helperprofile/${this.state.Helper_Mobile}`,
        state: { Mobile: this.state.Helper_Mobile },
      });
    }
  };
      ...
      inputs.push(
        <div style={{ fontFamily: "Kanit", fontSize: "1.5vw", width: "100%" }}>
          ...
          <input
            ...
            name={`input-${i}`}
            value={this.state.Detail[i].help}
          />
          <label>&nbsp;&nbsp;&nbsp;จำนวน&nbsp;&nbsp;&nbsp;</label>
          <input
            class="rounded-pill"
            ...
            name={`input-${i}`}
            value={this.state.Detail[i].count}
          />
          <label>&nbsp;&nbsp;&nbsp;สถานะ&nbsp;&nbsp;&nbsp;</label>
          ...
          <input
            ...
            name={`input-${i}`}
            value={this.state.Detail[i].status}
          />
          <div>
            ...
            <input
              ...
              name={`input-${i}`}
              value={this.state.Detail[i].option}
            />
          </div>
        </div>
      );
          ...
          <form onSubmit={this.onSubmit}>
            <div style={{ display: "flex", margin: "5% 0 0 3%" }}>
              ...
              <div style={{ width: "40%", textAlign: "left" }}>
                ...
                >
                  ขอความช่วยเหลือเมื่อ : {this.state.datet}
                </h1>
                <h1
                  style={{
                    color: "#707070",
            ...
            <div style={{ display: "flex", margin: "3% 0 0 3%" }}>
              ...
              <h1
                style={{
                  ...
                  fontSize: "1.5vw",
                  color: "#707070",
                  display: this.state.show_soi ? "block" : "none",
                }}
              >
                {" "}
                &nbsp; ซอย {this.state.Soi}
              ...
              <h1
                style={{
                  ...
                  fontSize: "1.5vw",
                  color: "#707070",
                  display: this.state.show_road ? "block" : "none",
                }}
              >
                {" "}
                &nbsp; ถนน {this.state.Road}
